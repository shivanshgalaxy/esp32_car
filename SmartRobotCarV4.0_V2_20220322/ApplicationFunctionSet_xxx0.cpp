...
void ApplicationFunctionSet::ApplicationFunctionSet_Init(void)
{
  ...
  //   /*清空串口缓存...*/
  // }
  Serial.println("elegoo:Smart Robot Car V4.0/TB6612/QMI8658C");
...
void ApplicationFunctionSet::ApplicationFunctionSet_SensorDataUpdate(void)
{
  ...
  { /*电压状态更新*/
    ...
    if (millis() - VoltageData_time > 100) //10ms 采集并更新一次
    {
      ...
      {
        VoltageData_number++;
        if (VoltageData_number == 50) //连续性多次判断最新的电压值...
        {
  ...
  //     float gyro, yaw;
  //     AppQMI8658C.QMI8658C_dveGetEulerAngles(&gyro, &yaw);
  //     Serial.println("\t");
  //     Serial.println(yaw);
  //     Serial.println("\t  ");
  //     Serial.println(gyro);
...
void ApplicationFunctionSet::ApplicationFunctionSet_Tracking(void)
{
  ...
  if (Application_SmartRobotCarxxx0.Functional_Mode == TraceBased_mode)
  {
    ...
    {
      print_time = millis();
      Serial.println("ITR20001_getAnaloguexxx_L=");
      Serial.println(getAnaloguexxx_L);
      Serial.println("ITR20001_getAnaloguexxx_M=");
      Serial.println(getAnaloguexxx_M);
      Serial.println("ITR20001_getAnaloguexxx_R=");
...
void ApplicationFunctionSet::ApplicationFunctionSet_IRrecv(void)
{
  ...
  if (true == IRrecv_en)
  {
    ...
    break;
    case /* constant-expression */ 15 ... 17:
    {
#if _is_print
      Serial.println(IRrecv_button);
#endif
...
void ApplicationFunctionSet::ApplicationFunctionSet_SerialPortDataAnalysis(void)
{
  ...
  if (c == '}') //数据帧尾部校验  数据帧格式 "{key:value}"
  {
    ...
    else if (!error) //检查反序列化是否成功
    {
      ...
      AppQMI8658C.QMI8658C_getMotion6(&ax, &ay, &az, &gx, &gy, &gz);
#if _is_print
        char toString[50];
